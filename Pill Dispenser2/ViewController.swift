...
//  Pill Dispenser2
//
//  Created by Raed Fayad on 2018-11-27.
...
class ViewController: UIViewController, UITextViewDelegate {
    ...
    func read(){
        ...
        let readFuture = self.dataCharacteristic?.read(timeout: 6)
        
        readFuture?.onSuccess { (_) in let s = String(data:(self.dataCharacteristic?.dataValue)!, encoding: .ascii)
            DispatchQueue.main.async {
                if let ASCIIstring = NSString(data: (self.dataCharacteristic?.dataValue)!, encoding: String.Encoding.utf8.rawValue) {
    ...
    func write(){
        ...
        let writeFuture = (self.valueToWriteTextField.text! as NSString).data(using: String.Encoding.utf8.rawValue)
        let b = self.dataCharacteristic?.write(data: writeFuture!, timeout: 5, type: .withoutResponse)
        b?.onSuccess(completion: { (_) in
            print("write succes")
        })
        b?.onFailure(completion: { (e) in
            print("write failed")
        })
    }
